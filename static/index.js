...
function openStaff() {
  ...
  var drawerBtn = document.getElementById("ManageStaffBtn");
...
function RoomMGMT() {
  var drawer = document.getElementById("RoomMenu");
  var drawerBtn = document.getElementById("ManageRoomBtn");
...
AddRoomBtn.addEventListener("click", () => {
  window.open("../static/pages/HTMLPages/AddRooms.html", "_self");
});
...
StaffCalendarButtonButton.addEventListener("click", () => {
...
document
  ...
  .addEventListener("input", searchReservations);
...
ViewHistoryButton.addEventListener("click", () => {
  window.open("../static/pages/HTMLPages/History.html", "_self");
});
...
Reviews.addEventListener("click", () => {
  window.open(
    "https://admin.booking.com/hotel/hoteladmin/extranet_ng/manage/reviews.html?hotel_id=11698782&lang=xu&list=with_comments&review_score=&guest_type=&guest_country=&topics=&search_term=&ses=e770cebcc0d63a248de5e3f7545c6031",
    "_blank"
  );
});
...
GMaps.addEventListener("click", () => {
  window.open(
    "https://www.google.com/maps/place/Eden's+Villa/@6.692692,-1.676091,17z/data=!3m1!4b1!4m9!3m8!1s0xfdb97604d7a121b:0x7e55fb4cf009f522!5m2!4m1!1i2!8m2!3d6.692692!4d-1.6735161!16s%2Fg%2F11sjy4syx0?entry=ttu&g_ep=EgoyMDI0MTAwMi4xIKXMDSoASAFQAw%3D%3D",
    "_blank"
  );
});
...
function updateRoomCounts() {
  fetchRooms()
    .then((rooms) => {
      const { totalRooms, availableRooms, occupiedRooms, notReadyRooms } =
        countRooms(rooms);
      ...
      .then((snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          if (
            ...
            reservation.ReservationHistory[1] &&
            reservation.ReservationHistory[1].checkout === true
          ) {
            checkOutCount++;
          }
        });
...
function drawMultiLineChart(reservations) {
  ...
  // Draw axes
  svg.innerHTML += `<line class="axis" x1="${padding}" y1="${
    svgHeight - padding
  }" x2="${svgWidth - padding}" y2="${svgHeight - padding}"></line>`;
  svg.innerHTML += `<line class="axis" x1="${padding}" y1="${padding}" x2="${padding}" y2="${
    svgHeight - padding
  }"></line>`;
  ...
  timeIntervals.forEach((hour, index) => {
    ...
    svg.innerHTML += `<text x="${x - 10}" y="${
      svgHeight - padding + 20
    }" class="legend">${hour}:00</text>`;
  });

  // Draw y-axis labels (frequency)
  for (let i = 0; i <= maxFrequency; i++) {
    const y = mapFrequencyToY(i, maxFrequency);
    svg.innerHTML += `<text x="${padding - 30}" y="${
      y + 5
    }" class="legend">${i}</text>`;
  }
  ...
  svg.innerHTML += `<text x="${svgWidth - 120}" y="${
    padding - 20
  }" class="legend">Yesterday</text>`;
  svg.innerHTML += `<rect x="${svgWidth - 150}" y="${
    padding - 30
  }" width="20" height="5" fill="#ff4136"></rect>`;
  svg.innerHTML += `<text x="${svgWidth - 120}" y="${
    padding - 5
  }" class="legend">Today</text>`;
  svg.innerHTML += `<rect x="${svgWidth - 150}" y="${
    padding - 15
  }" width="20" height="5" fill="#0074d9"></rect>`;
}
